(() => {
 ...
 function paint(){
  ...
  q('#existingMessage').textContent=state?.state==='pending'?'会社登録を受け付けました。販売元で契約を確認し、利用開始後にTOYA Oneを開けます。':state?.state==='suspended'?'会社の利用が停止中、または契約期限を過ぎています。会社の管理者に確認してください。':state?.state==='inactive'?'このアカウントは利用停止中です。会社の管理者に確認してください。':'登録済みです。TOYA Oneを開いて利用できます。';
