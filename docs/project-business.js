...
(() => {
 ...
 function renderProfile(){
  ...
  q('#pbCompanyFields').innerHTML=field('自社の正式名称','pbIssuerName',profile.issuer_name,'text','maxlength="160"')+field('住所','pbIssuerAddress',profile.address,'text','maxlength="500"')+field('電話番号','pbIssuerPhone',profile.phone,'text','maxlength="80"')+field('登録番号（登録している場合）','pbIssuerRegistration',profile.registration_number,'text','placeholder="T＋13桁の数字" maxlength="14"')+'<label for="pbIssuerBank">振込先</label><textarea id="pbIssuerBank" maxlength="1000" placeholder="銀行・支店・口座種別・口座番号・口座名義">'+esc(profile.bank_details||'')+'</textarea>';
 }
 ...
 function renderEditor(){
  ...
  if(q('#pbSaveDoc'))q('#pbSaveDoc').onclick=saveDocument;if(q('#pbIssueDoc'))q('#pbIssueDoc').onclick=issueDocument;
  q('#pbShowPreview').onclick=preview;
 ...
 function calculatedDocument(){
  if(!siteLoaded)throw new Error('先に一覧を更新して請求額を確認してください。');
 ...
 function updateTotals(){
  if(!editor)return;const host=q('#pbTotals');try{const {d,totals:t}=calculatedDocument();host.innerHTML='<div class="pb-metrics"><div><small>'+(d.kind==='progress'?'今回請求額（税別）':'小計（税別）')+'</small><b>'+yen(t.subtotal)+'</b></div><div><small>消費税 '+d.tax_rate+'%</small><b>'+yen(t.tax)+'</b></div><div><small>合計（税込）</small><b>'+yen(t.total)+'</b></div></div>'+(d.kind==='estimate'?'<p class="pb-estimate-profit">'+(t.cost===null?'原価未入力 '+t.missingCost+'項目／入力済み原価 '+yen(t.enteredCost):'見込原価 '+yen(t.cost)+' ／ 見込利益 '+yen(t.profit))+'</p>':'');t.lines.forEach((r,i)=>{const x=q('[data-amount="'+i+'"]');if(x)x.textContent='金額 '+yen(r.amount);});}
  catch(e){host.textContent=e.message;}
  if(q('#pbIssueDoc'))q('#pbIssueDoc').disabled=!editor.id||dirty||busy||!siteLoaded;
 ...
 async function issueDocument(){
  if(!editor?.id||dirty||!siteLoaded)return note('先に下書きを保存してください。',true);
