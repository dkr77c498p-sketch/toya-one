...
(() => {
 ...
 function mount(){
  ...
  card.innerHTML='<h2>自動積算表・見積書</h2><p class="note">これから施工する現場も、完工済みの現場も選べます。工事項目ごとに費用を積み上げて、見積書を作成します。</p><label for="epSite">積算する現場</label><select id="epSite"><option value="">現場を選択</option></select><div class="pb-actions"><button id="epNew" class="btn lime" type="button" disabled>＋ 積算表を作る</button><button id="epReload" class="btn light" type="button">一覧を更新</button></div><p id="epStatus" class="note" role="status" aria-live="polite"></p><div id="epSavedPlans"></div><div id="epEditor"></div><div id="estimateDocumentHost"></div><details id="epQuotes"><summary>この現場の見積書</summary><div id="epQuoteList"></div></details><button id="epIssuer" class="btn light pb-wide" type="button">発行者情報の設定を開く</button>';
  const anchor=q('#projectBusinessCard')||q('#siteSummaryCard');if(!anchor)return;anchor.before(card);
  q('#epSite').onchange=e=>{const next=e.target.value;if(!discard()||!closeQuote()){e.target.value=selected;return;}selected=next;plan=null;dirty=false;quoteRequest=null;renderPlan();renderLists();};
  ...
  const link=document.createElement('div');link.className='card';link.id='epMasterLink';link.innerHTML='<h2>自動積算表・見積書</h2><p class="note">施工予定・完工済みの現場の積算を作成・複製できます。</p><button class="btn dark pb-wide" type="button">積算表・見積書を開く</button>';link.querySelector('button').onclick=()=>{q('nav [data-page="homePage"]')?.click();card.scrollIntoView({block:'start',behavior:'smooth'});};q('#masterPage')?.prepend(link);
  refresh();
 }
 ...
 async function refresh(){
  ...
  try{const result=await Promise.all([read('sites',company),read('estimate_plans',company),read('project_documents',company,true)]);if(t!==ticket||identity()!==mine)return;
   ...
   q('#epSite').innerHTML='<option value="">現場を選択</option>'+optionSites();if(!selectedSite()){selected='';plan=null;dirty=false;renderPlan();}q('#epSite').value=selected;renderLists();note(dirty?'未保存の積算表を表示しています。':selected?'積算表を新しく作るか、保存済みの表を開いてください。':'現場を選んでください。完工済みの現場も一覧にあります。');await loadRates(company,mine);
 ...
 function renderLists(){
  ...
  q('#epSavedPlans').innerHTML=!selected?'':rows.length?'<details open><summary>保存済みの積算表（'+rows.length+'件）</summary>'+rows.map(p=>'<div class="pb-document-row"><div><b>'+esc(p.title)+'</b><p>'+esc(p.customer_name||'宛先未入力')+'</p><strong>'+(p.calculation?.complete?'見積 '+yen(p.calculation.price):'積算途中 / 入力済原価 '+yen(p.calculation?.known_cost||0))+'</strong></div><button class="btn light" type="button" data-ep-open="'+esc(p.id)+'">開く</button></div>').join('')+'</details>':'<p class="note">この現場の積算表はまだありません。</p>';
