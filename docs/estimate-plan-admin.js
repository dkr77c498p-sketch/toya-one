...
(() => {
 ...
 function renderSimplePlan(){
  ...
  host.innerHTML='<section class="ep-plan ep-simple"><ol class="ep-steps" aria-label="見積作成の進み方"><li data-ep-step="1"><b>1</b>工事情報</li><li data-ep-step="2"><b>2</b>明細入力</li><li data-ep-step="3"><b>3</b>確認・印刷</li></ol><div id="epStep1"><h3>1. どの工事の見積ですか？</h3><p class="note">工事名と見積先を入力してください。工事場所は後からでも入力できます。</p>'+field('工事名','epJobName',plan.site_name||sites.find(s=>s.id===plan.site_id)?.name||'','text','maxlength="200" placeholder="例：○○様邸 解体工事"'+(plan.site_id?' readonly':''))+field('見積先（会社名・お名前）','epCustomer',plan.customer_name,'text','maxlength="160" placeholder="例：○○建設株式会社 御中"')+field('工事場所（任意）','epJobAddress',plan.site_address||'','text','maxlength="500" placeholder="例：鹿児島市○○町1-2"')+'<details><summary>見積先住所・件名・条件を入れる</summary>'+field('見積先住所','epAddress',plan.customer_address,'text','maxlength="500"')+field('見積件名（空欄なら工事名）','epTitle',plan.title,'text','maxlength="200"')+'<label for="epQuoteNotes">備考・見積条件</label><textarea id="epQuoteNotes" maxlength="3000">'+esc(plan.quote_notes)+'</textarea></details><button id="epNextItems" type="button" class="btn lime pb-wide">次へ：明細を入力する →</button></div><div id="epStep2" hidden><h3>2. 工事の内容と金額を入れます</h3><p class="note">使う工事項目を開き、品名を選んで数量と単価を入力してください。</p><button id="epTemplate" type="button" class="btn light pb-wide">過去の見積をコピーして始める</button><div id="epGroups"></div><button id="epAddGroup" type="button" class="btn light pb-wide">＋ 工事項目を追加</button><details><summary>税率・社内メモ・その他の操作</summary><label for="epTax">消費税</label><select id="epTax"><option value="10">10%</option><option value="8">8%</option><option value="0">非課税・対象外</option></select><label for="epInternalNotes">社内メモ</label><textarea id="epInternalNotes" maxlength="3000">'+esc(plan.internal_notes)+'</textarea><button id="epPrint" type="button" class="btn light pb-wide">社内用の積算表を印刷</button><label for="epCopySite">この見積の複製先</label><select id="epCopySite"><option value="">新しい工事</option>'+optionSites()+'</select><button id="epCopy" type="button" class="btn light pb-wide">複製して作る</button></details><div id="epTotals" aria-live="polite"></div><button id="epCreateQuote" type="button" class="btn lime pb-wide">保存して見積書を確認する →</button><button id="epBackInfo" type="button" class="btn light pb-wide">← 工事情報に戻る</button></div><div id="epInputFooter"><p id="epFormStatus" class="note" role="status" aria-live="polite"></p><button id="epSave" type="button" class="btn light pb-wide">途中で保存する</button></div></section>';
 ...
 const quoteChoices=name=>/仮設/.test(name)?[['養生足場','㎡'],['防音シート張り','㎡'],['散水費','式']]:/産業廃棄物|処分/.test(name)?[['木くず処分','m³'],['コンクリート処分','m³'],['混合廃棄物処分','m³'],['産業廃棄物運搬費','m³']]:/解体/.test(name)?[['内部解体','㎡'],['上屋解体','㎡'],['基礎コンクリート解体','m³'],['土間コンクリート解体','m³']]:/回送|諸経費|値引/.test(name)?[['重機回送費','式'],['諸経費','式'],['値引き','式']]:/その他/.test(name)?[['解体跡整地','式'],['土嚢積み','式']]:[];
