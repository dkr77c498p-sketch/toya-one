...
(() => {
 ...
 function mount(){
  ...
  card.innerHTML='<h2>自動積算表・見積書</h2><p class="note">日報・登録単価・請負金額から自動集計します。完工済みの現場も、新しく日報を入力した現場も、ここに積み上がります。</p><div id="epQuantitySheets"></div><h3>日報からの原価・利益</h3><div id="epActualLedger"></div><label for="epSite">現場の内訳を見る</label><select id="epSite"><option value="">現場を選択</option></select><div id="epActualDetail"></div><details id="epPlannedEstimates"><summary>見積書の作成・保存済み見積</summary><p class="note">実績の積算表は上に自動表示されます。提出用の見積書を作るときに開いてください。</p><div class="pb-actions"><button id="epNew" class="btn lime" type="button" disabled>＋ 新しい見積を作る</button><button id="epReload" class="btn light" type="button">見積一覧を更新</button></div><p id="epStatus" class="note" role="status" aria-live="polite"></p><div id="epSavedPlans"></div><div id="epEditor"></div><div id="estimateDocumentHost"></div><details id="epQuotes"><summary>この現場の見積書</summary><div id="epQuoteList"></div></details><button id="epIssuer" class="btn light pb-wide" type="button">発行者情報の設定を開く</button></details>';
  const anchor=q('#projectBusinessCard')||q('#siteSummaryCard');if(!anchor)return;anchor.before(card);
  q('#epSite').onchange=e=>{const next=e.target.value;if(!discard()||!closeQuote()){e.target.value=selected;return;}selected=next;plan=null;dirty=false;quoteRequest=null;renderPlan();renderLists();note(selected?'見積を作る場合は、実績を取り込むか新しい見積を作成してください。':'現場を選択すると、その現場の見積を確認できます。');};
  q('#epNew').onclick=()=>{if(!ready||!selectedSite()||!discard()||!closeQuote())return;plan=C.draft(selectedSite());dirty=true;quoteRequest=null;renderPlan();};
 ...
 async function refresh(){
  if(busy||!identity())return;const mine=owner,t=++ticket,company=cloudProfile.company_id;ready=false;q('#epNew').disabled=true;note('積算表を読み込み中…');
  try{const result=await Promise.all([read('sites',company),read('estimate_plans',company),read('project_documents',company,true)]);if(t!==ticket||identity()!==mine)return;
   ...
   q('#epSite').innerHTML='<option value="">現場を選択</option>'+optionSites();if(!selectedSite()){selected='';plan=null;dirty=false;renderPlan();}q('#epSite').value=selected;renderLists();note(dirty?'未保存の積算表を表示しています。':selected?'見積を作る場合は、実績を取り込むか保存済みの見積積算を開いてください。':'現場を選択すると、その現場の見積を確認できます。');await loadRates(company,mine);
 ...
 function renderLists(){
  if(!q('#epSavedPlans'))return;q('#epNew').disabled=!ready||!selectedSite();
  const rows=plans.filter(p=>p.site_id===selected).sort((a,b)=>b.updated_at.localeCompare(a.updated_at));
  q('#epSavedPlans').innerHTML=!selected?'':rows.length?'<details open><summary>保存済みの見積積算（'+rows.length+'件）</summary>'+rows.map(p=>'<div class="pb-document-row"><div><b>'+esc(p.title)+'</b><p>'+esc(p.customer_name||'宛先未入力')+'</p><strong>'+(p.calculation?.complete?'見積 '+yen(p.calculation.price):'積算途中 / 入力済原価 '+yen(p.calculation?.known_cost||0))+'</strong></div><button class="btn light" type="button" data-ep-open="'+esc(p.id)+'">開く</button></div>').join('')+'</details>':'<p class="note">この現場の見積積算はまだ保存されていません。</p>';
  ...
  const quoteRows=quotes.filter(d=>d.site_id===selected).sort((a,b)=>b.created_at.localeCompare(a.created_at));
 ...
 function gather(){
  ...
  plan.title=q('#epTitle').value.trim();plan.customer_name=q('#epCustomer').value.trim();plan.customer_address=q('#epAddress').value.trim();plan.quote_notes=q('#epQuoteNotes').value;plan.internal_notes=q('#epInternalNotes').value;
 ...
 async function save(){
  let data;try{data=copy(gather());C.calculate(data);if(!data.title)throw new Error('積算表の名称を入力してください。');}catch(e){return note(e.message,true);}
 ...
 function duplicate(){
  if(!plan?.id||dirty)return note('先に積算表を保存してください。',true);
  const target=sites.find(s=>s.id===q('#epCopySite').value);if(!target||!closeQuote())return;
  plan=C.duplicate(plan,target);selected=target.id;q('#epSite').value=selected;dirty=true;quoteRequest=null;renderLists();renderPlan();note(target.name+'用の積算表を複製しました。内容を調整して保存してください。');
 }
 function print(){
  let html;try{html=C.printHTML(gather(),selectedSite()?.name||'');}catch(e){return note(e.message,true);}
 ...
 window.ToyaEstimatePlanUI={isBusy:()=>busy,getSites:()=>copy(sites),
  ...
  updateSites(next){if(!identity()||busy||!q('#epSite'))return;sites=copy(next).sort((a,b)=>(a.status==='active'?0:1)-(b.status==='active'?0:1)||a.name.localeCompare(b.name,'ja'));q('#epSite').innerHTML='<option value="">現場を選択</option>'+optionSites();q('#epSite').value=selected;q('#epNew').disabled=!ready||!selectedSite();},
  useActual(draft){if(!identity()||busy||!ready||!sites.some(s=>s.id===draft.site_id)||!discard()||!closeQuote())return false;plan=copy(draft);selected=plan.site_id;q('#epSite').value=selected;dirty=true;quoteRequest=null;renderLists();renderPlan();note('実績の費用を取り込みました。見積先・工事条件・諸経費・利益を確認して保存してください。');return true;}
 };
