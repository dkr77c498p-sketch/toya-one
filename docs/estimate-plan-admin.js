...
(() => {
 ...
 function renderLists(){
  ...
  q('#epSavedPlans').querySelectorAll('[data-ep-delete-plan]').forEach(b=>b.onclick=async()=>{const id=b.dataset.epDeletePlan,saved=plans.find(p=>p.id===id);if(!saved)return;if(!confirm('「'+saved.title+'」を削除しますか？\n元には戻せません。'))return;b.disabled=true;b.textContent='削除中';note('見積を削除しています…');try{const r=await cloudClient.from('estimate_plans').delete().eq('company_id',cloudProfile.company_id).eq('id',id).select('id');if(r.error)throw r.error;if(!r.data?.some(x=>x.id===id))throw new Error('削除結果を確認できませんでした。');plans=plans.filter(p=>p.id!==id);if(plan?.id===id){plan=null;dirty=false;q('#epEditor').innerHTML='';}renderLists();note('見積を削除しました。');}catch(e){b.disabled=false;b.textContent='削除';note('削除できませんでした：'+(e?.message||e),true);}});
