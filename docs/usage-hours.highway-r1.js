...
(() => {
 ...
 function syncEntrySites(entry,siteNames){
  ...
  const entered=next.allocations.filter(a=>a.minutes!==0||a.clock||(entry.kind==='labor'&&highwayAmount(a).present&&highwayAmount(a).value!==0));
 ...
 const engine=Object.freeze({highwayAmount,resolveLaborHighway,keepHighway,workMinutes,validateEntry,syncEntrySites,dispatchCrew,sameDispatchCrew,build,fuelRows,adjust,tools,attachments,key});
 ...
 function entryHTML(e){
  ...
  const al=e.allocations.map((a,i)=>{
   ...
   return '<div class="uh-allocation"><label>'+esc(a.site)+'</label><div class="uh-input">'+
    ...
    (kind==='labor'?'<label class="uh-highway">高速代（円）<input data-stepper="1" data-uh-highway="'+i+'" type="number" inputmode="decimal" min="0" max="1000000000" step="0.01" aria-label="'+pfx+' 高速代（円）" value="'+esc(a.highway??'')+'" placeholder="利用した金額を入力"></label><p class="note uh-highway-note">この現場分の合計額。同じ車の高速代は支払った人の欄に1回だけ。派遣・経費欄との重複に注意。</p>':'')+
  ...
  const message=highwayEntryError(e)||validateEntry(e)||'合計 '+Math.floor(total/60)+'時間'+total%60+'分';
 ...
 function updateRow(row){
  const e=state[row.dataset.uhId];q('.uh-note',row).textContent=highwayEntryError(e)||validateEntry(e)||'時間を記録しました。日報保存後に集計します。';
 }
 ...
 function install(){
  ...
  const style=document.createElement('style');style.id='uhInlineStyle';style.textContent=`
   ...
   .uh-inline .note{margin:8px 0 0}.uh-inline .uh-highway{display:block;margin-top:12px}.uh-inline .uh-highway-note{font-size:12px;line-height:1.5}#uhCard>details>summary{padding:0}#uhCard>details[open]>summary{padding-bottom:12px}
  ...
  q('#uhEnable').onchange=()=>{if(!q('#uhEnable').checked&&Object.values(state).some(e=>e.kind==='labor'&&e.allocations.some(a=>highwayAmount(a).present&&highwayAmount(a).value!==0))){q('#uhEnable').checked=true;alert('高速代が入力されています。金額を消さないため時間入力は有効のままです。不要な高速代は入力欄で確認してください。');return;}enabled=q('#uhEnable').checked;display();};
  q('#uhBreak').oninput=()=>{breakMinutes=Number(q('#uhBreak').value);};
  q('#uhReset').onclick=()=>{if(confirm('この画面の時間入力だけを消してやり直しますか？保存済みの日報は変わりません。')){const previous=state;state={};
  ...
  q('#uhDefault').onclick=()=>{
   ...
   sync(d).filter(e=>['labor','dispatch'].includes(e.kind)).forEach(e=>{e.allocations=[keepHighway(e,{site:d.site,minutes:m})];e.fromReportTime=true;});display();
  };
  ...
  document.addEventListener('toya-work-time-change',event=>{
   ...
   if(event.detail.apply){
    ...
    if(shift)sync(d).filter(e=>['labor','dispatch'].includes(e.kind)).forEach(e=>{e.allocations=[keepHighway(e,{site:d.site,minutes:shift.minutes,premium:structuredClone(shift.premium)})];e.fromReportTime=true;});
   }
   schedule();
  });
  ...
  const valid=window.validate;window.validate=function(d){
   ...
   if(d.usageHours&&[...document.querySelectorAll('.uh-inline:not([hidden]) [data-uh-highway]')].some(i=>!i.checkValidity())){alert('高速代は0以上の金額で入力してください。');return false;}
  ...
  const text=window.lineText;window.lineText=function(d){let t=text.apply(this,arguments);if(d.usageHours?.entries?.length)t+='\n\n■現場別の作業・使用時間\n'+d.usageHours.entries.map(e=>e.label+'：'+e.allocations.map(a=>a.site+' '+(a.minutes===null?'時間未入力':Math.floor(a.minutes/60)+'時間'+a.minutes%60+'分')+(workTime.describe(a.premium,{includeRates:false})?'（'+workTime.describe(a.premium,{includeRates:false})+'）':'')+(e.kind==='labor'&&highwayAmount(a).present?' ／ 高速代 '+(highwayAmount(a).issue?'要確認':highwayAmount(a).value.toLocaleString('ja-JP')+'円'):'')).join('／')).join('\n');return t;};
  document.addEventListener('change',event=>{
   const el=event.target;
