...
(() => {
 ...
 const phaseRow=(row={},i=0)=>'<div class="sp-phase" data-sp-phase="'+i+'"><div class="sp-phase-head"><b>出来高 '+(i+1)+'</b><button class="btn danger" type="button" data-sp-remove="'+i+'">削除</button></div><label>工事内容</label><input data-sp-key="label" maxlength="160" value="'+esc(row.label||'')+'" placeholder="例：内部解体工事"><div class="sp-grid"><div><label>対象月</label><input data-sp-key="target_month" type="month" value="'+esc(row.target_month||'')+'"></div><div><label>金額（円・税別）</label><input data-sp-key="amount" type="number" inputmode="numeric" min="0" step="1" value="'+esc(row.amount??'')+'"></div></div><label>状態</label><select data-sp-key="status"><option value="planned"'+(row.status!=='complete'?' selected':'')+'>途中出来高</option><option value="complete"'+(row.status==='complete'?' selected':'')+'>月末確定</option></select><label>内容・範囲（任意）</label><input data-sp-key="notes" maxlength="1000" value="'+esc(row.notes||'')+'" placeholder="工事範囲や補足"></div>';
 ...
 function updateTotals(){
  ...
  const total=P.totals(rows,q('#spContract')?.value),contractAmount=Number(q('#spContract')?.value||0),ordered=[...rows].filter(r=>r.target_month).sort((a,b)=>a.target_month.localeCompare(b.target_month));let cumulative=0;const history=ordered.map(r=>{cumulative+=Number(r.amount||0);return '<div class="sp-progress-line"><b>'+esc(r.target_month)+'</b><span>出来高 '+yen(r.amount||0)+'</span><span>累計 '+yen(cumulative)+'</span><span>残金 '+yen(Math.max(0,contractAmount-cumulative))+'</span></div>';}).join('');host.innerHTML='<div class="sp-total"><span>累計出来高</span><br><b>'+yen(total.total)+'</b><br><span>請負残金</span><br><b>'+yen(Math.max(0,contractAmount-total.total))+'</b></div>'+history+(total.difference===null?'':total.matches?'<p class="sp-ok">請負金額まで出来高計上済みです。</p>:'<p class="sp-warning">未計上の請負残金 '+yen(Math.max(0,total.difference))+'</p>');
 }
