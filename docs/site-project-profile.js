...
(() => {
 ...
 function updateTotals(){const host=q('#spTotals');if(!host)return;const total=P.totals(phaseValues(),q('#spContract')?.value);host.innerHTML='<div class="sp-total"><span>内訳合計</span><br><b>'+yen(total.total)+'</b></div>'+(total.difference===null?'':total.matches?'<p class="sp-ok">請負金額と一致しています。</p>':'<p class="sp-warning">請負金額との差額 '+yen(total.difference)+'</p>');}
 ...
 async function save(){
  ...
  try{
   ...
   if(identity()!==mine||selectedSite()?.id!==sid)return;const saved=r.data;if(!saved?.profile||!saved?.contract)throw new Error('保存結果を確認できませんでした。');profile={...blank(),...saved.profile,contract_breakdown:P.breakdown(saved.profile.contract_breakdown)};contract=saved.contract;render('保存しました。今後の積算・出来高の基礎情報として使えます。');document.dispatchEvent(new CustomEvent('toya-site-project-profile-saved',{detail:{siteId:sid}}));
