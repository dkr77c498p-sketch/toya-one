...
(() => {
 ...
 function mountAdmin(){
  ...
  if(!q('#ptRates')&&q('#masterPage')){const box=document.createElement('div');box.id='ptRates';box.className='card';box.innerHTML='<h2>小型機械・工具の時間単価（管理者用）</h2><p class="note">1台・1本あたりの時間単価です。未登録の単価を0円として確定しません。燃料込み・燃料別を選んで登録します。燃料は日報の燃料欄から1回計上します。過去の未確定計算にも反映します。</p><label>小型機械・工具<input id="ptToolName" type="text" list="ptTools" placeholder="登録名を選択・入力"></label><datalist id="ptTools"></datalist><label>1時間単価（円）<input id="ptToolRate" type="number" inputmode="decimal" min="0" step="0.01" placeholder="未登録"></label><label>燃料の扱い<select id="ptFuelBasis"><option value="">未選択</option><option value="included">燃料込み（記録済み燃料を差引）</option><option value="separate">燃料別</option></select></label><button id="ptSaveTool" class="btn dark" type="button">この時間単価を保存</button><p id="ptToolStatus" class="note" role="status"></p>';q('#masterPage').prepend(box);q('#ptSaveTool').onclick=saveTool;q('#ptToolName').onchange=()=>{const r=toolRows.find(r=>r.label===q('#ptToolName').value.trim());q('#ptToolRate').value=r?.hourly_rate??'';q('#ptFuelBasis').value=r?.fuel_included===true?'included':r?.fuel_included===false?'separate':'';};}
 }
 ...
 async function saveTool(){
  ...
  const basis=q('#ptFuelBasis').value;if(!['included','separate'].includes(basis)){q('#ptToolStatus').textContent='燃料込み・燃料別を選んでください。';return;}const included=basis==='included';
  if(!confirm(label+'\n1時間 '+yen(n)+(included?'（燃料込み）':'（燃料別）')+'で登録しますか？'))return;
  const mine=ident(),company=cloudProfile.company_id,b=q('#ptSaveTool');b.disabled=true;
  try{const old=toolRows.find(r=>r.label===label);let req=old?cloudClient.from('small_tool_rate_master').update({hourly_rate:n,fuel_included:included,active:true,updated_at:new Date().toISOString()}).eq('company_id',company).eq('id',old.id).eq('updated_at',old.updated_at):cloudClient.from('small_tool_rate_master').insert({company_id:company,label,hourly_rate:n,fuel_included:included,active:true});const r=await req.select('id,label,hourly_rate');if(r.error)throw r.error;if(r.data?.length!==1)throw new Error('単価が同時に変更されました。画面を開き直してください。');if(ident()!==mine)return;q('#ptToolStatus').textContent=label+'：1時間 '+yen(n)+' を保存しました。';lastRead=0;editingAmount=false;writing=false;await load(true);q('#sfRefresh')?.click();}
