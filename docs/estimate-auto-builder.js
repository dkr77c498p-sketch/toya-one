(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.ToyaEstimateAuto=api;})(typeof window==='object'?window:globalThis,function(){
 ...
 const foundationFactors={wood:{base:.15,perFloor:.04},lightSteel:{base:.2,perFloor:.03},steel:{base:.23,perFloor:.05},rc:{base:.44,perFloor:.12}};
 function autoFoundation(kind,area,floors){const f=foundationFactors[kind],m2=num(area),n=Math.max(1,Math.round(num(floors)||1));return f&&m2?round(m2/n*(f.base+f.perFloor*(n-1)),2):'';}
 ...
 function normalize(raw={}){const d={...defaults(),...raw};if(num(d.area_m2))d.area_tsubo=round(num(d.area_m2)/TSUBO,2);else if(num(d.area_tsubo))d.area_m2=round(num(d.area_tsubo)*TSUBO,2);if(String(d.foundation_m3??'').trim()==='')d.foundation_m3=autoFoundation(d.kind,d.area_m2,d.floors);return d;}
 function build(raw){
  ...
  demolition.push(line('基礎コンクリート解体',a.foundation_m3,'m³',r.foundation,'建物面積・階数・構造別の平均から自動計算'),line('土間コンクリート解体',a.slab_m3,'m³',r.slab),line('外構撤去',a.exterior_m2,'㎡',''),line('庭石撤去',a.garden_m3,'m³',''),line('植栽撤去',a.plants_m3,'m³',''),line('CB撤去',a.cb_m2,'㎡',''),line('内部残置物撤去',a.residual_m3,'m³',10000));
  ...
  const based='TOYA標準300㎡表から面積比例',w=r.waste,wr=r.wasteRates,waste=m2&&!interior?[line('産業廃棄物運搬費（コンクリート以外）',m2*w.haul,'m³',wr.haul,based),line('コンクリート運搬費',m2*w.concrete,'m³',wr.concreteHaul,based),line('木くず処分費',m2*w.wood,'m³',wr.wood,based),line('ガラス・陶磁器くず処分費',m2*w.glass,'m³',wr.glass,based),line('廃プラスチック処分費',m2*w.plastic,'m³',wr.plastic,based),line('紙・繊維くず処分費',m2*w.paper,'m³',wr.paper,based),line('石膏ボード処分費',m2*w.board,'m³',wr.board,based),line('コンクリート・アスファルト処分費',m2*w.concrete,'m³',wr.concrete,based),line('がれき類処分費',m2*w.rubble,'m³',wr.rubble,based),line('金属くず買取控除',m2*w.metal,'t',wr.metal,'37円/kg／'+based),line('混合廃棄物処分費',m2*w.mixed,'m³',wr.mixed,based)]:[];
 ...
 return {TSUBO,structureRates,foundationFactors,autoFoundation,defaults,normalize,build,percentRows,round};
});
