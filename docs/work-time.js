...
(() => {
 ...
 const overtimeFactor=holiday=>holiday===true?1.3:1.25;
 function resolvePremium(p,settings){
  return p&&p.overtimeMultiplier==null&&typeof settings?.holiday==='boolean'?{...p,overtimeMultiplier:overtimeFactor(settings.holiday)}:p;
 }
 ...
 function classify(start,end,settings){
  ...
  const premium=resolvePremium({overtimeMinutes:0,nightMinutes:0,holidayNightMinutes:0,overtimeMultiplier:settings.overtimeMultiplier??null},settings);
 ...
 function read(){
  return {version:1,nextDay:q('#wtNextDay').checked,holiday:q('#wtHoliday').checked,
   overtimeMultiplier:overtimeFactor(q('#wtHoliday').checked),
 ...
 function restore(d,mode){
  ...
  const s=d?.workTime||{};q('#wtNextDay').checked=s.nextDay===true;q('#wtHoliday').checked=s.holiday===true;
 ...
 function install(){
  ...
  card.addEventListener('change',event=>{
   const holidayChanged=event.target.id==='wtHoliday';
   if(holidayChanged&&manualPremium){const night=manualPremium.nightMinutes+manualPremium.holidayNightMinutes;manualPremium.nightMinutes=q('#wtHoliday').checked?0:night;manualPremium.holidayNightMinutes=q('#wtHoliday').checked?night:0;q('#wtNight').value=manualPremium.nightMinutes/60;q('#wtHolidayNight').value=manualPremium.holidayNightMinutes/60;}
   update(false,holidayChanged);
  });
