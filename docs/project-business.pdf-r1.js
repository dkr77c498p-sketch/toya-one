...
(() => {
 ...
 let owner='',sites=[],docs=[],profile={},contract=null,rates=[],customers=[],siteId='',editor=null,dirty=false,busy=false,ticket=0,kind='invoice';
 let closeIssueConfirmation=null,closeDocumentPreview=null,closeDeleteConfirmation=null;
 const canDeleteInvoice=d=>!!d?.id&&!!d.updated_at&&['invoice','progress'].includes(d.kind)&&['draft','void'].includes(d.status)&&d.issued_at==null&&d.document_number==null&&d.number_year==null&&d.number_seq==null;
 ...
 function issueAvailable(){
  ...
  if(busy||window.ToyaEstimatePlanUI?.isBusy()){note('保存処理中です。完了してから、もう一度押してください。',true);return false;}
  return true;
 }
 // A browser may suppress window.confirm(). Keep this decision in the app so
 // tapping the issue button always opens a visible, explicit confirmation.
 function confirmDocumentIssue(d,amount){
  if(closeIssueConfirmation)return Promise.resolve(false);
  return new Promise(resolve=>{
   const opener=q('#pbIssueDoc')||document.activeElement,dialog=document.createElement('dialog');let finished=false;
   dialog.id='pbIssueConfirmation';dialog.setAttribute('aria-labelledby','pbIssueTitle');dialog.setAttribute('aria-describedby','pbIssueExplanation');
   dialog.innerHTML='<h3 id="pbIssueTitle">'+esc(E.kinds[d.kind])+'を確定します</h3><dl><dt>宛先</dt><dd>'+esc(d.customer_name)+'</dd><dt>件名</dt><dd>'+esc(d.subject)+'</dd></dl><div class="pb-metrics"><div><small>合計（税込）</small><b>'+yen(amount)+'</b></div></div><p id="pbIssueExplanation">確定後は内容が固定され、書類番号が付きます。</p><button id="pbIssueApprove" class="btn lime pb-wide" type="button">確定して番号を付ける</button><button id="pbIssueCancel" class="btn light pb-wide" type="button" autofocus>戻る</button>';
   const finish=accepted=>{if(finished)return;finished=true;closeIssueConfirmation=null;if(dialog.open)dialog.close();dialog.remove();if(opener?.isConnected)opener.focus({preventScroll:true});resolve(accepted);};
   closeIssueConfirmation=()=>finish(false);
   q('#pbIssueApprove',dialog).onclick=()=>finish(true);q('#pbIssueCancel',dialog).onclick=()=>finish(false);
   dialog.addEventListener('cancel',event=>{event.preventDefault();finish(false);});dialog.addEventListener('close',()=>finish(false));
   document.body.append(dialog);
   try{dialog.showModal();q('#pbIssueCancel',dialog).focus();}catch(e){finish(false);note('確認画面を開けませんでした。画面を再読み込みしてください。',true);}
  });
 }
 ...
 function clear(){closeIssueConfirmation?.();closeDeleteConfirmation?.();closeDocumentPreview?.();siteLoaded=false;profileDirty=false;kind='invoice';owner='';sites=[];docs=[];profile={};rates=[];customers=[];siteId='';editor=null;dirty=false;ticket++;q('#projectBusinessCard')?.remove();q('#pbMasterLink')?.remove();q('#pbHomeActions')?.remove();if(q('#estimateDocumentHost'))q('#estimateDocumentHost').innerHTML='';}
 function mount(){
 ...
 function renderOverview(){
  ...
  q('#pbOverview').innerHTML='<label for="pbSettingsSiteSelect">現場を選択</label><select id="pbSettingsSiteSelect">'+[...sites].sort((a,b)=>(a.status==='active'?0:1)-(b.status==='active'?0:1)||a.name.localeCompare(b.name,'ja')).map(x=>'<option value="'+esc(x.id)+'" '+(x.id===s.id?'selected':'')+'>'+esc(x.name)+(x.completed_on?'（完工）':'')+'</option>').join('')+'</select><div class="pb-state">'+(s.completed_on?'完工済み '+esc(s.completed_on):s.status==='active'?'工事中':'過去・未整理')+'</div><label for="pbContractAmount">請負金額（税別）</label><div class="pb-actions"><input id="pbContractAmount" type="number" min="0" step="1" inputmode="numeric" value="'+(amount===null?'':amount)+'" placeholder="例：3200000"><button id="pbContractSave" class="btn lime" type="button">入力・変更</button></div><div class="pb-metrics"><div><small>請負金額（税別）</small><b>'+ (amount===null?'未登録':yen(amount))+'</b></div><div><small>請求済額（税別）</small><b>'+yen(billed)+'</b></div><div><small>未請求額（税別）</small><b>'+(amount===null?'—':yen(amount-billed))+'</b></div></div><p class="note">請求は請負金額の内訳として管理します。売上・利益はホームの現場別集計で確認できます。</p>';q('#pbSettingsSiteSelect').onchange=async e=>{if(dirty&&!confirm('書類の未保存の入力を閉じて現場を切り替えますか？')){e.target.value=siteId;return;}siteId=e.target.value;editor=null;dirty=false;q('#pbSite').value=siteId;await loadSite();};q('#pbContractSave').onclick=saveContractAmount;
 }
 async function saveContractAmount(){
  const s=site(),input=q('#pbContractAmount');if(!s||!input||busy)return;const amount=Number(input.value);
  if(!Number.isSafeInteger(amount)||amount<0)return note('請負金額を0円以上の整数で入力してください。',true);
  await action(async()=>{let r;if(contract)r=await cloudClient.from('revenues').update({amount,updated_at:new Date().toISOString()}).eq('id',contract.id).eq('company_id',cloudProfile.company_id).select('*').single();else r=await cloudClient.from('revenues').insert({company_id:cloudProfile.company_id,site_id:s.id,revenue_date:date(),revenue_type:'contract',description:'請負金額',amount}).select('*').single();if(r.error)throw r.error;contract=r.data;renderOverview();note(s.name+'の請負金額を'+yen(amount)+'（税別）で保存しました。');document.dispatchEvent(new CustomEvent('toya-project-document-changed',{detail:{site_id:s.id,kind:'contract'}}));});
 }
 ...
 async function completeSimpleEstimate(){
  ...
  const mine=owner;
  if(!await confirmDocumentIssue(d,E.total(d.items,d.tax_rate).total))return;
  if(identity()!==mine||!editor)return;
  try{if(JSON.stringify(calculatedDocument().d)!==JSON.stringify(d))return note('内容が変わりました。もう一度確認してから確定してください。',true);}catch(e){return note(e.message,true);}
  if(!issueAvailable())return;
  await action(async()=>{
   note('見積書を確定しています…');
 ...
 function renderEditor(scroll=true){
  ...
  if(q('#pbCustomerMaster'))q('#pbCustomerMaster').onchange=e=>{const x=customers.find(v=>v.id===e.target.value);if(!x)return;q('#pbCustomer').value=x.name||'';q('#pbCustomerAddress').value=[x.postal_code?'〒'+x.postal_code:'',x.address].filter(Boolean).join(' ');dirty=true;updateTotals();};
  if(q('#pbSaveCustomer'))q('#pbSaveCustomer').onclick=async()=>{const name=q('#pbCustomer').value.trim(),address=q('#pbCustomerAddress').value.trim();if(!name)return note('提出先の会社名を入力してください。',true);const existing=customers.find(x=>x.name===name);let r;if(existing)r=await cloudClient.from('business_customers').update({address,active:true,updated_at:new Date().toISOString()}).eq('id',existing.id).select('*').single();else r=await cloudClient.from('business_customers').insert({company_id:cloudProfile.company_id,name,address,active:true}).select('*').single();if(r.error)return note('取引先を保存できませんでした：'+r.error.message,true);if(existing)customers=customers.map(x=>x.id===r.data.id?r.data:x);else customers.push(r.data);note('提出先「'+name+'」を取引先に保存しました。');renderEditor(false);};
  ...
  window.ToyaJapaneseInput.bind(host,e=>{if(e.target.closest('#pbFields')){dirty=true;updateTotals();}},note);
  if(q('#pbEditBreakdown'))q('#pbEditBreakdown').onclick=async()=>{if(dirty)await saveDocument();if(dirty||!identity()||!editor)return;const saved=copy(editor);editor=null;dirty=false;host.innerHTML='';window.ToyaEstimatePlanUI?.returnToInput(saved);};
 ...
 function gather(){
  ...
  const d=editor;d.document_date=q('#pbDocumentDate').value;d.transaction_start=q('#pbTransactionStart').value||null;d.transaction_end=q('#pbTransactionEnd').value||null;d.due_date=q('#pbDueDate')?.value||null;d.valid_until=q('#pbValidUntil')?.value||null;d.customer_name=q('#pbCustomer').value.trim();d.customer_address=q('#pbCustomerAddress').value.trim();d.subject=q('#pbSubject').value.trim();d.notes=q('#pbNotes').value;if(d.kind==='estimate'){d.work_period=q('#pbWorkPeriod').value;d.estimate_conditions=q('#pbEstimateConditions').value;}d.tax_rate=Number(q('#pbTaxRate').value);
 ...
 async function issueDocument(){
  ...
  if(!await confirmDocumentIssue(d,d.total))return;
  if(identity()!==mine||!editor)return;
  if(!documentReady()||dirty||editor.id!==d.id||editor.updated_at!==d.updated_at)return note('内容が変わりました。もう一度確認してから確定してください。',true);
  if(!issueAvailable())return;
 ...
 function preview(){
  ...
  closeDocumentPreview?.();const opener=document.activeElement,dialog=document.createElement('dialog');dialog.id='pbPreview';dialog.innerHTML='<div class="pb-preview-toolbar"><b>'+E.kinds[d.kind]+'プレビュー</b><div>'+ (d.kind==='estimate'&&d.status==='draft'?'<button id="pbPreviewConditions" class="btn light" type="button">見積条件を変更</button>':'')+''+(d.kind==='estimate'?'<button id="pbPdf" class="btn dark" type="button">PDF保存・共有</button><button id="pbPrint" class="btn light" type="button">印刷（従来）</button>':'<button id="pbPrint" class="btn dark" type="button">印刷・PDF保存</button>')+'<button id="pbPreviewClose" class="btn light" type="button">閉じる</button></div></div><p class="note">'+(d.status==='draft'&&d.kind!=='estimate'?'今は下書きです。正式な請求書にするには、この画面を閉じて「下書きを保存」→「内容を確定・採番」を押してください。請求番号が付き、下書き表示が消えます。':d.kind==='estimate'?'PDF保存・共有で「TOYAONE.pdf」を作成します。下書きの表示は残ります。従来の印刷では端末側のURLが付く場合があります。':'印刷画面でPDFに保存できます。下書きには「下書き」と表示します。')+'</p><div class="pb-preview-viewport" tabindex="0" role="region" aria-label="書類のプレビュー"><div class="pb-preview-stage"><iframe title="書類の印刷プレビュー" scrolling="no" sandbox="allow-same-origin allow-modals"></iframe></div></div>';document.body.append(dialog);
  ...
  const pdfHTML=E.printHTML(d);frame.srcdoc=pdfHTML;q('#pbPrint').onclick=()=>{frame.contentWindow.focus();frame.contentWindow.print();};q('#pbPreviewClose').onclick=closeDocumentPreview;
  if(d.kind==='estimate'){if(window.ToyaQuotePDF)pdfController=window.ToyaQuotePDF.mount({dialog,button:q('#pbPdf'),html:pdfHTML,isCurrent:()=>!closed&&identity()===pdfOwner});else q('#pbPdf').onclick=()=>alert('PDF作成機能を読み込めませんでした。入力を保存後、画面を更新してください。');}
  if(q('#pbPreviewConditions'))q('#pbPreviewConditions').onclick=()=>{closeDocumentPreview();const fields=q('#pbReviewFields'),input=q('#pbEstimateConditions');if(fields)fields.open=true;if(input){input.scrollIntoView({block:'center'});input.focus({preventScroll:true});}};
