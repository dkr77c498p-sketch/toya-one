...
(async()=>{
 ...
 try{
  ...
  assert.equal(doc.subject,name,'工事名に未入力の「見積」を自動追加しない');
  assert.equal(doc.estimate_snapshot.title,name);assert.equal(q('#pbSubject').value,name);
  ...
  assert.doesNotMatch(print,/家屋内部残置物撤去工事 見積/);
