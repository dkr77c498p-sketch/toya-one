...
async function setup(role='admin',active=true){
 ...
 w.ToyaCompanyAccess={allows:()=>true};w.ToyaSiteCostSummaryEngine=S;w.applyCloudRoleUI=()=>{};w.confirm=()=>true;w.Element.prototype.scrollIntoView=function(){};w.HTMLDialogElement.prototype.showModal=function(){this.open=true};w.HTMLDialogElement.prototype.close=function(){this.open=false};
...
(async()=>{
 ...
 const detailNode=q('#epActualDetail').firstElementChild,tableNode=q('#alRows').firstElementChild,reads=calls.length;
 const expanded=q('#epActualDetail details');if(expanded)expanded.open=true;
 s.intervals.forEach(fn=>fn());w.dispatchEvent(new w.Event('pageshow'));w.document.dispatchEvent(new w.Event('visibilitychange'));await new Promise(r=>setTimeout(r,40));
 assert.equal(q('#epActualDetail').firstElementChild,detailNode);assert.equal(q('#alRows').firstElementChild,tableNode);if(expanded)assert.equal(expanded.open,true);assert.equal(calls.length,reads);console.log('PASS background refresh preserves visible nodes and expanded details');
 q('#alRefresh').click();assert.equal(q('#epActualDetail').firstElementChild,detailNode);assert.equal(q('#alRows').firstElementChild,tableNode);await waitFor(()=>!q('#alRefresh').disabled);console.log('PASS manual refresh retains content until replacement is ready');
