...
(async()=>{
 ...
 assert.equal(q('#epAutoFoundation').value,'45');
 q('#epAutoFloors').value='2';q('#epAutoFloors').dispatchEvent(new w.Event('input',{bubbles:true}));
 assert.equal(q('#epAutoFoundation').value,'28.5');
 ...
 assert.equal(byLabel('木くず処分費').querySelector('[data-quote-key="quantity"]').value,'52.3');
 assert.equal(byLabel('金属くず買取控除').querySelector('[data-quote-key="quote_price"]').value,'-37000');
