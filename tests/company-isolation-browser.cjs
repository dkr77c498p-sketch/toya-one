 ...
 try{
  ...
  await page.addInitScript(({initial})=>{
   ...
   window.supabase={createClient(){return{
    ...
    rpc:async name=>({data:name==='toya_access'?(sessionStorage.getItem('qa-license-blocked')?{state:'suspended',message:'会社の契約が停止中です。'}:{state:'ready',internal:true,plan:'complete',features:{daily:true,invoice:true,estimate:true}}):name==='toya_employee_admin'?{company_code:'AABBCCDDEEFF',internal:true,employees:[]}:[],error:null})
   };}};
