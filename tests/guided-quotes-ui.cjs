...
(async()=>{
 ...
 u.put('#pbNotes','変更した見積条件');s.w.confirm=()=>false;const before=s.calls.length;q('#pbIssueDoc').click();q('#pbIssueCancel').click();await settle();assert.equal(s.calls.length,before);assert.equal(s.db.project_documents[1].status,'draft');
 s.w.confirm=()=>true;q('#pbIssueDoc').click();q('#pbIssueApprove').click();await settle();assert.equal(s.db.project_documents[1].status,'issued');assert.equal(s.db.project_documents[1].notes,'変更した見積条件');assert.ok(q('#pbPreview'));assert.match(q('#pbPreview iframe').srcdoc,/変更した見積条件/);assert.match(q('#pbPreview iframe').srcdoc,/17,600/);assert.equal(JSON.stringify({sites:s.db.sites,revenues:s.db.revenues}),baseline);s.dom.window.close();
